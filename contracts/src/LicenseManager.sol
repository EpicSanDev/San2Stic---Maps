...
contract LicenseManager is AccessControl, Pausable {
    ...
    function setRecordingLicense(uint256 _recordingId, LicenseType _licenseType, string calldata _attribution)
        external
        whenNotPaused
    {
    ...
    function reportLicenseViolation(uint256 _recordingId, string calldata _reason) external whenNotPaused {
        require(_recordingId > 0, "Invalid recording ID");
    ...
    function setUserLicensePreference(LicenseType _licenseType, bool _preferred) external whenNotPaused {
        require(licenses[_licenseType].isActive, "License type not active");
    ...
    function checkLicenseCompatibility(LicenseType _originalLicense, LicenseType _derivativeLicense)
        external
        view
        returns (bool compatible, string memory reason)
    {
    ...
    function getRecordingLicense(uint256 _recordingId)
        external
        view
        returns (LicenseType licenseType, string memory attribution, LicenseInfo memory licenseInfo)
    {
    ...
    function getUserLicensePreferences(address _user) external view returns (LicenseType[] memory preferredLicenses) {
        uint256 count = 0;
