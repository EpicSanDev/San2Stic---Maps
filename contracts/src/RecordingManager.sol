...
contract RecordingManager is AccessControl, ReentrancyGuard, Pausable {
    ...
    function addRecording(
        ...
        recordingStats[_recordingIdCounter] = RecordingStats({upvotes: 0, downvotes: 0, totalRating: 0, ratingCount: 0});
    ...
    function updateRecordingMetadata(
        ...
        require(
            recordingMetadata[_recordingId].status == ModerationStatus.PENDING,
            "Cannot update approved/rejected recordings"
        );
    ...
    function getRecording(uint256 _recordingId)
        external
        view
        returns (RecordingCore memory core, RecordingMetadata memory metadata, RecordingStats memory stats)
    {
        require(_recordingId > 0 && _recordingId <= _recordingIdCounter, "Recording does not exist");
    ...
    function getRecordingsByLocation(
        ...
        for (uint256 i = 1; i <= _recordingIdCounter && count < _limit; i++) {
            ...
            if (
                core.isActive && recordingMetadata[i].status == ModerationStatus.APPROVED && core.latitude >= _minLat
                    && core.latitude <= _maxLat && core.longitude >= _minLng && core.longitude <= _maxLng
            ) {
                if (skipped >= _offset) {
                    result[count] = i;
                    count++;
    ...
    function isLocationUsed(int256 _latitude, int256 _longitude, string calldata _ipfsHash)
        external
        view
        returns (bool)
    {
