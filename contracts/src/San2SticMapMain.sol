...
contract San2SticMapMain is AccessControl, ReentrancyGuard, Pausable {
    ...
    function batchVoteOnRecordings(
    ...
    function batchSetRecordingLicenses(
        ...
        require(
            _recordingIds.length == _licenseTypes.length && 
            _licenseTypes.length == _attributions.length,
    ...
    function getRecordingWithAllData(uint256 _recordingId) external view returns (
        RecordingManager.RecordingCore memory core,
