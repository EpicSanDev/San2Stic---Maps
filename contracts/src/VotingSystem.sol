...
contract VotingSystem is AccessControl, ReentrancyGuard, Pausable {
    ...
    event ModerationActionTaken(
        uint256 indexed recordingId, address indexed moderator, ModerationStatus action, string reason
    );
    ...
    function voteOnRecording(uint256 _recordingId, bool _isUpvote, address _recordingCreator)
        external
        recordingExists(_recordingId)
        notRecordingCreator(_recordingId, _recordingCreator)
        whenNotPaused
        nonReentrant
    {
        ...
        votes.push(
            Vote({voter: msg.sender, recordingId: _recordingId, isUpvote: _isUpvote, timestamp: block.timestamp})
        );

        emit RecordingVoted(_recordingId, msg.sender, _isUpvote);
    }

    function rateRecording(uint256 _recordingId, uint8 _score, address _recordingCreator)
        external
        recordingExists(_recordingId)
        notRecordingCreator(_recordingId, _recordingCreator)
        whenNotPaused
        nonReentrant
    {
        ...
        ratings.push(Rating({rater: msg.sender, recordingId: _recordingId, score: _score, timestamp: block.timestamp}));
    ...
    function moderateRecording(uint256 _recordingId, ModerationStatus _status, string calldata _reason)
        external
    ...
    function batchModerateRecordings(
        ...
        require(
            _recordingIds.length == _statuses.length && _statuses.length == _reasons.length, "Array length mismatch"
        );
    ...
    function proposeModeration(uint256 _recordingId, ModerationStatus _proposedStatus, string calldata _reason)
        external
        recordingExists(_recordingId)
        whenNotPaused
    {
        require(
            userReputation[msg.sender] >= MIN_REPUTATION_TO_MODERATE, "Insufficient reputation to propose moderation"
        );
    ...
    function getRecordingRating(uint256 _recordingId)
        external
        view
    ...
    function getUserVotingHistory(address _user, uint256 _offset, uint256 _limit)
        external
        view
        returns (Vote[] memory)
    {
